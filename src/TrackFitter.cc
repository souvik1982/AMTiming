...
TrackFitter::TrackFitter(std::string name, double inTime, double outTime, double delay)
{
  ...
  v_h_t1out_.push_back(new TH1F(("h_t1out_"+name_).c_str(), (";TrackFitter "+name_+" t1out").c_str(), 100, 0, 10000));
  v_h_t2out_.push_back(new TH1F(("h_t2out_"+name_).c_str(), (";TrackFitter "+name_+" t2out").c_str(), 100, 0, 10000));
}
  ...
  if (delay_>0 && inTime_>0 && outTime_>0)
  {
    if (nCombinations_!=-999 && nTracks_!=-999)
    {
      if (t1in_.at(0)!=-999 && t2in_.at(0)!=-999)
      {
        ...
        t2out_.at(0)=std::max(t2in_.at(0)+delay_, std::max(t1out_.at(0)+nCombinations_*inTime_, t1out_.at(0)+nTracks_*outTime_));
